...
![Library Ecore Model](fr.unice.i3s.sigma.docs/figures/Library.ecore.svg "Library Ecore Model")
...
The delegating model of Sigma therefore expects two delege methods with following signatures:
...
* In Scala
      ...
      deg getLoans(self: Book): List[Loan]
...
![Sigma Delegates Setup](fr.unice.i3s.sigma.docs/figures/SigmaDelegatesSetup.png "Sigma Delegates Setup")
...
## EMF Model Builders (Scala only) ##

## Model Validation (Scala only) ##

### Abstrcat Syntax ###

### Quick Fixes ###

## Model to Text Transformation (Scala only) ##

## Model to Model Transformation (Scala only) ##
...
* `skip-ui-tests` - do not run UI tests. In order to skip all tests add `-Dmaven.test.skip=true` to the `./build.sh`.