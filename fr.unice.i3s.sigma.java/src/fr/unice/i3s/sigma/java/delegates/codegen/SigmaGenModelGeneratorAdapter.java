...
import fr.unice.i3s.sigma.java.delegates.AbstractSigmaDelegate;
...
import fr.unice.i3s.sigma.java.delegates.JSigmaInvocationDelegate;
import fr.unice.i3s.sigma.java.delegates.JSigmaInvocationDelegateFactory;
import fr.unice.i3s.sigma.java.delegates.JSigmaSettingDelegate;
import fr.unice.i3s.sigma.java.delegates.JSigmaSettingDelegateFactory;
import fr.unice.i3s.sigma.java.delegates.JSigmaValidationDelegate;
import fr.unice.i3s.sigma.java.delegates.JSigmaValidationDelegateFactory;
import fr.unice.i3s.sigma.java.delegates.StaticValidationDelegate;

public class SigmaGenModelGeneratorAdapter extends GenBaseGeneratorAdapter {

	private final JSigmaDelegateDomain domain;

	public SigmaGenModelGeneratorAdapter(JSigmaDelegateDomain domain,
	...
	private void processModel(GenModel genModel) {
		JSigmaInvocationDelegateFactory invocationFactory = createInvocationDelegateFactory();
		JSigmaSettingDelegateFactory settingFactory = createSettingDelegateFactory();
		JSigmaValidationDelegateFactory validationFactory = createValidationDelegateFactory();
		...
		for (GenPackage genPkg : genModel.getGenPackages()) {
			for (GenClass genClazz : genPkg.getGenClasses()) {
				...
				for (GenOperation genOperation : genClazz.getGenOperations()) {
					...
					JSigmaInvocationDelegate delegate = invocationFactory
				...
				for (GenFeature genFeature : genClazz.getGenFeatures()) {
					...
					JSigmaSettingDelegate delegate = settingFactory
				...
				for (final String constraint : getEClassConstraints(clazz)) {
					JSigmaValidationDelegate delegate = validationFactory
	...
	protected String generateOperationInvocationMethodCall(GenClass genClazz,
			JSigmaInvocationDelegate delegate) {
	...
	protected String generateFeatureInvocationMethodCall(GenClass genClazz,
			JSigmaSettingDelegate delegate) {
		// GenModel model = genClazz.getGenModel();
	...
	private String generateValidationInvocationMethodCall(GenClass genClazz,
			JSigmaValidationDelegate delegate) {
		// GenModel model = genClazz.getGenModel();
	...
	protected JSigmaInvocationDelegateFactory createInvocationDelegateFactory() {
		return new JSigmaInvocationDelegateFactory();
	}

	protected JSigmaSettingDelegateFactory createSettingDelegateFactory() {
		return new JSigmaSettingDelegateFactory();
	}

	protected JSigmaValidationDelegateFactory createValidationDelegateFactory() {
		return new JSigmaValidationDelegateFactory();
	}
