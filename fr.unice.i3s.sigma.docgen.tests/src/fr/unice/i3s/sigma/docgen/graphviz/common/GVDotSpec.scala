...
import fr.unice.i3s.sigma.docgen.graphviz.common.GVOutputType._
import fr.unice.i3s.sigma.docgen.graphviz.common.GVDot._
...
class GVDotSpec extends FlatSpec with MustMatchers with MockitoSugar {
  ...
  it must "detect incompatible version" in {
    ...
    val thrown = evaluating { dot.validate.get } must produce[UnsupportedVersionException]
    thrown.getMessage must equal(s"Expected >= $COMPATIBLE_VERSION, got 1.28")
  }

  it must "correctly compose the cmdline arguments" in {
    val executorMock = mock[Executor]
    when(executorMock.execute(anyString())).thenReturn(Success(""))
    val dot = new GVDot("pathToDot") {
      override val executor = executorMock
    }
